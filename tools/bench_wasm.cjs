...
// builds can be diffed. MIR is generated by the native build and, when absent,
// compiled once on demand with the pinned stanc; compilation is not timed.
"use strict";
const fs = require("fs");
const os = require("os");
...
function loadModel(M, stem, dataFile) {
  ...
  const mirPath = path.join(fixtures, stem + ".tmir.sexp");
  let mir;
  if (fs.existsSync(mirPath)) {
    mir = fs.readFileSync(mirPath, "utf8");
  } else {
    const executable = process.platform === "win32" ? "stanc.exe" : "stanc";
    const stanc = path.resolve(
        process.env.STANC || path.join(repo, "deps", "stanc3", executable));
    if (!fs.existsSync(stanc))
      throw new Error("missing generated MIR and pinned compiler: " + stanc);
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "stanli-wasm-bench-"));
    try {
      const source = path.join(root, stem + ".stan");
      fs.copyFileSync(path.join(fixtures, stem + ".stan"), source);
      const compiled = spawnSync(
          stanc, ["--O1", "--debug-optimized-mir", source],
          {encoding: "utf8"});
      if (compiled.error) throw compiled.error;
      if (compiled.status !== 0)
        throw new Error("stanc failed: " +
                        (compiled.stderr || compiled.status));
      mir = compiled.stdout;
    } finally {
