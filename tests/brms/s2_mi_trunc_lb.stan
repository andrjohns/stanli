...
data {
  ...
  array[N_yposmi] real lb_yposmi;  // lower truncation bounds;
...
model {
  ...
  if (!prior_only) {
    ...
    for (n in 1:N_yposmi) {
      target += normal_lpdf(Yl_yposmi[n] | mu_yposmi[n], sigma_yposmi) -
        normal_lccdf(lb_yposmi[n] | mu_yposmi[n], sigma_yposmi);
    }
